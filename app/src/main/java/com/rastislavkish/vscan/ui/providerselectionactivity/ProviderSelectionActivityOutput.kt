...
@Serializable
data class ProviderSelectionActivityOutput(
    val provider: Int,
    ) {

    companion object {

        fun fromIntent(intent: Intent?, caller: String): ProviderSelectionActivityOutput {
            val key="result"
            ...
            if (intent!=null) {
                ...
                if (extras!=null) {
                    ...
                    try {
                        return Json.decodeFromString<ProviderSelectionActivityOutput>(json)
                        }
                    catch (e: Exception) {
